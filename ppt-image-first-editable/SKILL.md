...
- **Phase C — 分层生成 + HTML 文字编辑（可编辑路径）**
  Phase A 完成后**主动询问用户是否要可编辑文字版**。若用户同意，沿用 Phase A 的 Stage 1-3 规划文件，但**重新走 Stage 4**：双轨生成"带装饰、留白文字区"的背景图 → 文字作为外挂图层在 HTML 编辑器里调整 → 一键渲染成原生可编辑 PPTX。文字始终是真 TextBox（PPT 里可改）。
...
- **双轨生成不能省**：必须先出"完整稿"，再以完整稿为 imagegen edit target 出"擦字稿"。**不要直接一步生成"留白稿"**——模型会偷加占位文字 / 留白位置不对 / 整页空洞。
- **第 2 稿必须以第 1 稿为 edit target**：必须先 `view_image` 第 1 稿，再调 imagegen，prompt 写"以刚刚显示的这张图片作为唯一编辑目标"。不要只写本地路径。
